    ...
    <a-row v-for="(set,setIndex) in tableList" :key="setIndex">
      <a-row class="title">
        <a-col :span='3' class="winnerBox">
          <div class="winner" v-show="set.winner === 1">{{ 'Win' }}</div>
        </a-col>
        <a-col :span='9' class="leftBox">
          <div class="left">{{ `SET ${setIndex + 1}` }}</div>
        </a-col>
        <a-col :span='9' class="rightBox">
          <div class="right">{{ getGameType(set.type) }}</div>
        </a-col>
        <a-col :span='3' class="winnerBox">
          <div class="winner" v-show="set.winner === 2">{{ 'Win' }}</div>
        </a-col>
      </a-row>
        ...
        <a-row v-for="(player,playerIndex) in leg.playerList" :key="playerIndex" class="playerBox">
          <a-col :span='4'>
            <div class="playerImg">
              ...
              <div class="playerName">{{ player.homeName }}</div>
            </div>
          </a-col>
          <a-col :span='3'>{{ player.homePpd }}</a-col>
          <a-col :span='3'>{{ player.homeScores }}</a-col>
          <a-col :span='4' class="winStyle">
              <div class="WinStyle WinBox">{{ leg.winner === 1?'win':'lose' }}</div>
              <div class="WinStyle">{{ leg.winner === 2?'win':'lose' }}</div>
          </a-col>
          ...
          LEG: [
            {
              ...
              playerList: [
                {
                  homeImg: require("@/assets/1.jpg"),
                  homeName: "李逍遥",
                  homePpd: 25.12,
                  homeScores: 25.12,
                  awayImg: require("@/assets/1.jpg"),
                  awayName: "李逍遥",
                  awayPpd: 25.12,
                  awayScores: 25.12,
                },
